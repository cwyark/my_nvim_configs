...
# Simple fzf installer
# - Detect OS and arch (x86_64 -> amd64, aarch64 -> arm64)
# - Get latest release tag from GitHub
# - Build asset filename like: fzf-0.66.1-linux_arm64.tar.gz
# - Download and extract/move fzf binary into ~/.local/bin/fzf
...
if [ -z "${ASSET_NAME:-}" ]; then
  err "Could not download a matching fzf asset for $OS/$ARCH (release $TAG_NAME)"
  exit 1
fi
...
*.tar.gz | *.tgz)
  ...
  BIN_PATH=$(find "$TMPDIR" -type f -name fzf -print -quit || true)
  ;;
*.zip)
  if command -v unzip >/dev/null 2>&1; then
    ...
    err "zip archive found but unzip is not installed"
    exit 1
  fi
  ;;
*)
...
if [ -z "${BIN_PATH:-}" ] || [ ! -s "$BIN_PATH" ]; then
  err "fzf binary not found after extracting asset"
  ls -la "$TMPDIR" || true
  exit 1
fi
